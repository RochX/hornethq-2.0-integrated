...
useEffect(() => {
  const fetchData = async () => {
    try {
      ...
      const uniqueClassIds = Array.from(new Set(response.data.map((course) => course.course_id)));
...
}, [carouselData.length, setCarouselData]);
...
const handlePrev = () => {
  setCurrentSlide(currentSlide > 0 ? currentSlide - 1 : carouselData.length - 1);
};
...
useEffect(() => {
  const intervalId = setInterval(() => {
    console.log("Handling next slide...");
    handleNext();
  }, 5000);
...
if (!carouselData.length) {
  console.log("Carousel data is empty. Rendering loading...");
...
const courseType = carouselData[currentSlide].course_id.split("-")[0];
  ...
  return (
    <div className="carousel">
      ...
      {[...new Set(carouselData[currentSlide].offering.map((offering) => offering.faculty_name))].map((facultyName, index) => (
